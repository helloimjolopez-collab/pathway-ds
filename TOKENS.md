...
|---|---|---|
| `themes/light.css` | **290** | **Yes** — this is the colour contract |
| `themes/midnight.css` | 290 | Yes — the *same* names, different values |
| `type.css` | 41 | Yes |
| `layout.css` | 39 | Yes |
| `motion.css` | 17 | Yes |
...
## Why 400 and not 1,066

If you add up every declaration across the token folder you get 1,066. That number is
real and it is not the contract:

- **The two theme files declare the same 290 names**, once each. One name, two values,
