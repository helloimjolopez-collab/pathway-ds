...
export function createPrimitiveColors() {
  const groups = {};
  for (const [name, token] of Object.entries(tokens)) {
    if (token.type !== "color" || token.path[0] !== "primitive-color") continue;
    const palette = token.path[1];
    if (!groups[palette]) groups[palette] = [];
    groups[palette].push({
      name,
      value: token.value,
      step: token.path.slice(2).join("-") || name,
    });
  }

  const container = document.createElement("div");

  for (const [palette, colors] of Object.entries(groups)) {
    ...
    for (const color of colors) {
      ...
      tokenName.style.cssText =
        "font-family: monospace; font-size: 11px; color: #555; word-break: break-all;";
