...
METRICS are component-specific (`--contextual-layout-units-*`). Text and Icon
were merged into Foreground, because a control is one interactive surface and
its label and icon must not resolve through two ramps that can drift apart.
...
Annotations use the design system's text styles (`Body/S/Regular`, `Label/Small/Medium`). They don't use raw font properties. They use `Foreground/Static/Neutral/Base` as their colour token. They sit at the same depth on the canvas as the thing they annotate, never on top of it.
