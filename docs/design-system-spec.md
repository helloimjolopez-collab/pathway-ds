...
| Action semantics | `Foreground/Action/Primary/On Subtle/Rest` | Interactive. Rest / Hover / Pressed, with ONE Disabled per tier |
...
Lowercase with dots in JSON (`semantic-color.light-mode.foreground.static.neutral.base`); style-dictionary emits hyphens for CSS (`--semantic-color-foreground-static-neutral-base`). Components consume the CSS variable form.
...
| No `foreground.*` inverse track | MEDIUM | White spinners on brand-filled buttons have no matching semantic token. |
