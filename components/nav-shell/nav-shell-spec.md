...
- Active tab: `foreground.action.primary.on-subtle.rest` (#345499), font-weight 600
- Active indicator: 2px bar at bottom, `fill.action.primary.strong.rest` (#4364b6), border-radius 2px 2px 0 0
- Inactive tab: `foreground.static.neutral.base` (#313131), font-weight 500
...
Each section has:
1. **SectionHeading:** 14px/600 uppercase, `foreground.static.neutral.base` (#313131), letter-spacing 0.6px, padding-vertical 6px
...
│           color: foreground.static.info.on-subtle = #464080
└── CardBody (14px/400/20px, foreground.static.neutral.base = #313131)
```

Card icon color variants (32×32 container):
- **Accent/amethyst:** bg `fill.accent_amethyst.light` (#f4f2fa)
- **Positive/green:** bg `fill.static.positive.subtle` (#dff6e2)
- **Info/blue:** bg `fill.static.brand.faint` (#eef2fb)
- **Warning:** bg `fill.static.attention.subtle` (#fef2de)
