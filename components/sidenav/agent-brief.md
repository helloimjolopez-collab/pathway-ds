...
- Two widths: **250 px expanded** and **72 px collapsed (icon-only rail)**.
...
3. **Trail-collapsed state**: when a grouper is closed and one of its children is the active destination, the grouper itself shows **Active styling**: same fill (`#a0b5e629`), same text (`#1b2d57`), same icon (`#2d4889`), stripe visible. This applies whether the sidebar is 250 px or 72 px. See [§6](./sidenav-spec.md#6-state-matrix).
...
8. **The NavHeader (expand/collapse control) renders at the TOP of the nav at ALL breakpoints ≥768 px**, in both the 250 px and 72 px states. It is absent only on mobile (<768 px). At 72 px the action icon is **centered**; at 250 px it is **right-aligned** in Slot.RowEnd. The icon is 12×12 (`right_panel_open` when expanded, `left_panel_open` when collapsed), colour `Foreground/Action/Secondary/Rest` (`#6b6b6b`). A 1 px `Stroke/Static/Neutral/Base` (`#f6f6f6`) divider sits **below** the NavHeader. It is sticky-positioned so it never scrolls out of view. See [§9](./sidenav-spec.md#9-navheader-collapse--expand-control).

9. **Responsive contract**:
   - **≥1024 px**: SideNav is in flow (`flex-shrink: 0`), 250 px or 72 px. Content shifts.
   - **768–1023 px**: 72 px rail in flow by default. Tapping expand → 250 px overlay with scrim.
   - **<768 px**: SideNav **hidden by default**. Hamburger in TopNav reveals a 250 px overlay with scrim. **No 72 px rail at all on mobile.**
...
- [ ] SideNav uses `#fafafa` background and matches the 250 px / 72 px width contract
...
- [ ] At <768 px: nav is hidden by default, hamburger in TopNav reveals 250 px overlay with scrim
