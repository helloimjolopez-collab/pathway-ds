...
| 🎨 **Design tokens (CSS variables)** | [`src/tokens/themes/light.css`](https://github.com/helloimjolopez-collab/pathway-ds/blob/main/src/tokens/themes/light.css) | Style-Dictionary output. The colour contract: one name per token, mode by selector. Spacing is `layout.css`, type is `type.css`. |
