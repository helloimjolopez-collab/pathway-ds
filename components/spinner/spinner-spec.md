...
Spinner's colour API is **locked to the `foreground.*` semantic family**. The component exposes a `tone` prop (which family) and an `emphasis` prop (how strong). The two together resolve to a single semantic token; the token resolves to a primitive; the primitive resolves to a hex. Consumers never see the primitive or the hex.
...
| `brand` | `foreground.action.primary.on-subtle.rest` | `brand-500` | `#345499` |
| `info` | `foreground.static.info.on-subtle` | `amethyst-600` | `#464080` |
| `warning` | `foreground.static.attention.on-subtle` | `saffron-600` | `#51432c` |
| `danger` | `foreground.static.negative.on-subtle` | `red-700` | `#722121` |
| `negative` | `foreground.static.negative.on-subtle` | `red-700` | `#722121` |
| `positive` | `foreground.static.positive.on-subtle` | `green-700` | `#174f26` |
| `accent-amethyst` | `foreground.static.info.on-subtle` | `amethyst-600` | `#464080` |
| `accent-jade` | `foreground.static.accent.jade.on-subtle` | `jade-600` | `#135349` |
| `accent-seabreeze` | `foreground.static.accent.seabreeze.on-subtle` | `seabreeze-700` | `#185167` |
...
> control is inert for that tone. `Fill/Static/Brand` is not a substitute: a fill token
...
> `foreground.static.negative.on-subtle`. The two names are kept because both are in
...
```
  ...
  token-path   = semantic.light-mode.color.foreground.{tone}.{emphasis}
```
...
| `accent-jade` | `light` | `--semantic-color-foreground-static-accent-jade-on-subtle` | `foreground.static.accent.jade.on-subtle` |
