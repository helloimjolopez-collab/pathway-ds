...
Fill/Primary uses `foreground.static.neutral.mono`. Fill/Static/Negative uses `foreground.static.neutral.mono`.
...
- Fill/Primary label on blue background: `foreground.static.neutral.mono` is white in both modes, so it is guaranteed ≥ 4.5:1 against `fill.action.primary` by token design.
...
3. the token contract (`themes/*.css`, `layout.css`, `type.css`, `motion.css`, `breakpoints.css`) — all `--semantic-*` CSS variables. `tokens.css` was retired 2026-09-03.
...
- tokens from the token contract (`themes/*.css`, `layout.css`, `type.css`, `motion.css`, `breakpoints.css`) (never invent or hardcode colours)
