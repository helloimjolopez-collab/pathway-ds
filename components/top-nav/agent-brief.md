...
- **Background: `Fill/Brand/Contrast` → `#2d4889`** (brand blue, NOT dark navy)
...
2. **Use the brand-blue background.** `Fill/Brand/Contrast` → `#2d4889`. Not dark navy (`#0a1223`). Not custom.
...
3. **Slot layout (left → right) is fixed:**
   ...
   - **Row End:** Search (48×48 wrapper → 32×32 circle with `cornerradius/full: 64px` + `search` icon) · Desktop: 2× `notifications` bells (48×48 each) | Tablet+Mobile: `more_vert` (48×48) · Profile (32×32 circle, `Fill/Accent_Amethyst/Base` `#dcd9ef`, initials in `#221e3f`)
...
6. **Profile is amethyst, always.** Background `Fill/Accent_Amethyst/Base` (`#dcd9ef`), text `Foreground/Accent/Amethyst/Contrast` (`#221e3f`). Initials, not photos.
