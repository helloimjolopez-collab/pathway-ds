...
form came from `tokens.css`, removed 2026-09-03, and no longer resolves. Do not write
new code against it.
...
There are 111 classes in `type.css`, one per text style, and a mobile media query
...
Tokens flow one way: **Figma → this folder → the `src/tokens/` contract + `tokens.js`**. See the repo `CLAUDE.md` §2 for the full pipeline.
